/*
 * Copyright (C) 2016 - Niklas Baudy, Ruben Gees, Mario Đanić and contributors
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package com.vanniktech.emoji.facebook.category

import com.vanniktech.emoji.facebook.FacebookEmoji

internal object ActivitiesCategoryChunk0 {
  internal val EMOJIS: List<FacebookEmoji> = listOf(
    FacebookEmoji(String(intArrayOf(0x1F383), 0, 1), listOf("jack_o_lantern"), 7, 1, false),
    FacebookEmoji(String(intArrayOf(0x1F384), 0, 1), listOf("christmas_tree"), 7, 2, false),
    FacebookEmoji(String(intArrayOf(0x1F386), 0, 1), listOf("fireworks"), 7, 9, false),
    FacebookEmoji(String(intArrayOf(0x1F387), 0, 1), listOf("sparkler"), 7, 10, false),
    FacebookEmoji(String(intArrayOf(0x1F9E8), 0, 1), listOf("firecracker"), 54, 26, false),
    FacebookEmoji(String(intArrayOf(0x2728), 0, 1), listOf("sparkles"), 60, 24, false),
    FacebookEmoji(String(intArrayOf(0x1F388), 0, 1), listOf("balloon"), 7, 11, false),
    FacebookEmoji(String(intArrayOf(0x1F389), 0, 1), listOf("tada"), 7, 12, false),
    FacebookEmoji(String(intArrayOf(0x1F38A), 0, 1), listOf("confetti_ball"), 7, 13, false),
    FacebookEmoji(String(intArrayOf(0x1F38B), 0, 1), listOf("tanabata_tree"), 7, 14, false),
    FacebookEmoji(String(intArrayOf(0x1F38D), 0, 1), listOf("bamboo"), 7, 16, false),
    FacebookEmoji(String(intArrayOf(0x1F38E), 0, 1), listOf("dolls"), 7, 17, false),
    FacebookEmoji(String(intArrayOf(0x1F38F), 0, 1), listOf("flags"), 7, 18, false),
    FacebookEmoji(String(intArrayOf(0x1F390), 0, 1), listOf("wind_chime"), 7, 19, false),
    FacebookEmoji(String(intArrayOf(0x1F391), 0, 1), listOf("rice_scene"), 7, 20, false),
    FacebookEmoji(String(intArrayOf(0x1F9E7), 0, 1), listOf("red_envelope"), 54, 25, false),
    FacebookEmoji(String(intArrayOf(0x1F380), 0, 1), listOf("ribbon"), 6, 60, false),
    FacebookEmoji(String(intArrayOf(0x1F381), 0, 1), listOf("gift"), 6, 61, false),
    FacebookEmoji(
      String(intArrayOf(0x1F397), 0, 1), listOf("reminder_ribbon"), 7, 24, false,
      variants = listOf(
        FacebookEmoji(String(intArrayOf(0x1F397, 0xFE0F), 0, 2), emptyList<String>(), 7, 24, false),
      ),
    ),
    FacebookEmoji(
      String(intArrayOf(0x1F39F), 0, 1), listOf("admission_tickets"), 7, 29, false,
      variants = listOf(
        FacebookEmoji(String(intArrayOf(0x1F39F, 0xFE0F), 0, 2), emptyList<String>(), 7, 29, false),
      ),
    ),
    FacebookEmoji(String(intArrayOf(0x1F3AB), 0, 1), listOf("ticket"), 7, 41, false),
    FacebookEmoji(
      String(intArrayOf(0x1F396), 0, 1), listOf("medal"), 7, 23, false,
      variants = listOf(
        FacebookEmoji(String(intArrayOf(0x1F396, 0xFE0F), 0, 2), emptyList<String>(), 7, 23, false),
      ),
    ),
    FacebookEmoji(String(intArrayOf(0x1F3C6), 0, 1), listOf("trophy"), 9, 1, false),
    FacebookEmoji(String(intArrayOf(0x1F3C5), 0, 1), listOf("sports_medal"), 9, 0, false),
    FacebookEmoji(String(intArrayOf(0x1F947), 0, 1), listOf("first_place_medal"), 43, 53, false),
    FacebookEmoji(String(intArrayOf(0x1F948), 0, 1), listOf("second_place_medal"), 43, 54, false),
    FacebookEmoji(String(intArrayOf(0x1F949), 0, 1), listOf("third_place_medal"), 43, 55, false),
    FacebookEmoji(String(intArrayOf(0x26BD), 0, 1), listOf("soccer"), 59, 11, false),
    FacebookEmoji(String(intArrayOf(0x26BE), 0, 1), listOf("baseball"), 59, 12, false),
    FacebookEmoji(String(intArrayOf(0x1F94E), 0, 1), listOf("softball"), 43, 60, false),
    FacebookEmoji(String(intArrayOf(0x1F3C0), 0, 1), listOf("basketball"), 8, 0, false),
    FacebookEmoji(String(intArrayOf(0x1F3D0), 0, 1), listOf("volleyball"), 10, 5, false),
    FacebookEmoji(String(intArrayOf(0x1F3C8), 0, 1), listOf("football"), 9, 8, false),
    FacebookEmoji(String(intArrayOf(0x1F3C9), 0, 1), listOf("rugby_football"), 9, 9, false),
    FacebookEmoji(String(intArrayOf(0x1F3BE), 0, 1), listOf("tennis"), 7, 60, false),
    FacebookEmoji(String(intArrayOf(0x1F94F), 0, 1), listOf("flying_disc"), 43, 61, false),
    FacebookEmoji(String(intArrayOf(0x1F3B3), 0, 1), listOf("bowling"), 7, 49, false),
    FacebookEmoji(String(intArrayOf(0x1F3CF), 0, 1), listOf("cricket_bat_and_ball"), 10, 4, false),
    FacebookEmoji(String(intArrayOf(0x1F3D1), 0, 1), listOf("field_hockey_stick_and_ball"), 10, 6, false),
    FacebookEmoji(String(intArrayOf(0x1F3D2), 0, 1), listOf("ice_hockey_stick_and_puck"), 10, 7, false),
    FacebookEmoji(String(intArrayOf(0x1F94D), 0, 1), listOf("lacrosse"), 43, 59, false),
    FacebookEmoji(String(intArrayOf(0x1F3D3), 0, 1), listOf("table_tennis_paddle_and_ball"), 10, 8, false),
    FacebookEmoji(String(intArrayOf(0x1F3F8), 0, 1), listOf("badminton_racquet_and_shuttlecock"), 10, 48, false),
    FacebookEmoji(String(intArrayOf(0x1F94A), 0, 1), listOf("boxing_glove"), 43, 56, false),
    FacebookEmoji(String(intArrayOf(0x1F94B), 0, 1), listOf("martial_arts_uniform"), 43, 57, false),
    FacebookEmoji(String(intArrayOf(0x1F945), 0, 1), listOf("goal_net"), 43, 52, false),
    FacebookEmoji(String(intArrayOf(0x26F3), 0, 1), listOf("golf"), 59, 27, false),
    FacebookEmoji(
      String(intArrayOf(0x26F8), 0, 1), listOf("ice_skate"), 59, 31, false,
      variants = listOf(
        FacebookEmoji(String(intArrayOf(0x26F8, 0xFE0F), 0, 2), emptyList<String>(), 59, 31, false),
      ),
    ),
    FacebookEmoji(String(intArrayOf(0x1F3A3), 0, 1), listOf("fishing_pole_and_fish"), 7, 33, false),
    FacebookEmoji(String(intArrayOf(0x1F93F), 0, 1), listOf("diving_mask"), 43, 46, false),
    FacebookEmoji(String(intArrayOf(0x1F3BD), 0, 1), listOf("running_shirt_with_sash"), 7, 59, false),
    FacebookEmoji(String(intArrayOf(0x1F3BF), 0, 1), listOf("ski"), 7, 61, false),
    FacebookEmoji(String(intArrayOf(0x1F6F7), 0, 1), listOf("sled"), 39, 4, false),
    FacebookEmoji(String(intArrayOf(0x1F94C), 0, 1), listOf("curling_stone"), 43, 58, false),
    FacebookEmoji(String(intArrayOf(0x1F3AF), 0, 1), listOf("dart"), 7, 45, false),
    FacebookEmoji(String(intArrayOf(0x1FA80), 0, 1), listOf("yo-yo"), 55, 1, false),
    FacebookEmoji(String(intArrayOf(0x1FA81), 0, 1), listOf("kite"), 55, 2, false),
    FacebookEmoji(String(intArrayOf(0x1F52B), 0, 1), listOf("gun"), 30, 35, false),
    FacebookEmoji(String(intArrayOf(0x1F3B1), 0, 1), listOf("8ball"), 7, 47, false),
    FacebookEmoji(String(intArrayOf(0x1F52E), 0, 1), listOf("crystal_ball"), 30, 38, false),
    FacebookEmoji(String(intArrayOf(0x1FA84), 0, 1), listOf("magic_wand"), 55, 5, false),
    FacebookEmoji(String(intArrayOf(0x1F3AE), 0, 1), listOf("video_game"), 7, 44, false),
    FacebookEmoji(
      String(intArrayOf(0x1F579), 0, 1), listOf("joystick"), 31, 52, false,
      variants = listOf(
        FacebookEmoji(String(intArrayOf(0x1F579, 0xFE0F), 0, 2), emptyList<String>(), 31, 52, false),
      ),
    ),
    FacebookEmoji(String(intArrayOf(0x1F3B0), 0, 1), listOf("slot_machine"), 7, 46, false),
    FacebookEmoji(String(intArrayOf(0x1F3B2), 0, 1), listOf("game_die"), 7, 48, false),
    FacebookEmoji(String(intArrayOf(0x1F9E9), 0, 1), listOf("jigsaw"), 54, 27, false),
    FacebookEmoji(String(intArrayOf(0x1F9F8), 0, 1), listOf("teddy_bear"), 54, 42, false),
    FacebookEmoji(String(intArrayOf(0x1FA85), 0, 1), listOf("pinata"), 55, 6, false),
    FacebookEmoji(String(intArrayOf(0x1FAA9), 0, 1), listOf("mirror_ball"), 55, 35, false),
    FacebookEmoji(String(intArrayOf(0x1FA86), 0, 1), listOf("nesting_dolls"), 55, 7, false),
    FacebookEmoji(
      String(intArrayOf(0x2660), 0, 1), listOf("spades"), 58, 49, false,
      variants = listOf(
        FacebookEmoji(String(intArrayOf(0x2660, 0xFE0F), 0, 2), emptyList<String>(), 58, 49, false),
      ),
    ),
    FacebookEmoji(
      String(intArrayOf(0x2665), 0, 1), listOf("hearts"), 58, 51, false,
      variants = listOf(
        FacebookEmoji(String(intArrayOf(0x2665, 0xFE0F), 0, 2), emptyList<String>(), 58, 51, false),
      ),
    ),
    FacebookEmoji(
      String(intArrayOf(0x2666), 0, 1), listOf("diamonds"), 58, 52, false,
      variants = listOf(
        FacebookEmoji(String(intArrayOf(0x2666, 0xFE0F), 0, 2), emptyList<String>(), 58, 52, false),
      ),
    ),
    FacebookEmoji(
      String(intArrayOf(0x2663), 0, 1), listOf("clubs"), 58, 50, false,
      variants = listOf(
        FacebookEmoji(String(intArrayOf(0x2663, 0xFE0F), 0, 2), emptyList<String>(), 58, 50, false),
      ),
    ),
    FacebookEmoji(
      String(intArrayOf(0x265F), 0, 1), listOf("chess_pawn"), 58, 48, false,
      variants = listOf(
        FacebookEmoji(String(intArrayOf(0x265F, 0xFE0F), 0, 2), emptyList<String>(), 58, 48, false),
      ),
    ),
    FacebookEmoji(String(intArrayOf(0x1F0CF), 0, 1), listOf("black_joker"), 0, 15, false),
    FacebookEmoji(String(intArrayOf(0x1F004), 0, 1), listOf("mahjong"), 0, 14, false),
    FacebookEmoji(String(intArrayOf(0x1F3B4), 0, 1), listOf("flower_playing_cards"), 7, 50, false),
    FacebookEmoji(String(intArrayOf(0x1F3AD), 0, 1), listOf("performing_arts"), 7, 43, false),
    FacebookEmoji(
      String(intArrayOf(0x1F5BC), 0, 1), listOf("frame_with_picture"), 32, 25, false,
      variants = listOf(
        FacebookEmoji(String(intArrayOf(0x1F5BC, 0xFE0F), 0, 2), emptyList<String>(), 32, 25, false),
      ),
    ),
    FacebookEmoji(String(intArrayOf(0x1F3A8), 0, 1), listOf("art"), 7, 38, false),
    FacebookEmoji(String(intArrayOf(0x1F9F5), 0, 1), listOf("thread"), 54, 39, false),
    FacebookEmoji(String(intArrayOf(0x1FAA1), 0, 1), listOf("sewing_needle"), 55, 27, false),
    FacebookEmoji(String(intArrayOf(0x1F9F6), 0, 1), listOf("yarn"), 54, 40, false),
    FacebookEmoji(String(intArrayOf(0x1FAA2), 0, 1), listOf("knot"), 55, 28, false),
  )
}
